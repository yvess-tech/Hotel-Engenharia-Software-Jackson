...
const hospedagens = [
  ...
  },
  {
    ...
    quarto: "407",
    tipo: "Check-out",
    data: "13/09/2026 11:00",
    status: "Finalizado",
  },
]

export default function CheckInOut() {
  const [modal, setModal] = useState<"checkin" | "checkout" | "fatura" | null>(
    null,
  )
  const [selected, setSelected] = useState<typeof pendingCheckins[0] | null>(
    null,
  )
              ...
              >
                <div>
                  <p
                    ...
                    style={{
                      fontFamily: '"Inria Serif:Bold", serif',
                      color: "#374151",
                    }}
                  >
                    {c.hospede}
              ...
              >
                <div>
                  ...
                  <p className="text-xs mt-1" style={{ color: "#d97706" }}>
                    Saída: {c.checkout} · Saldo: {c.saldo}
                  </p>
                </div>
                <div className="flex gap-1 flex-col">
                  <Btn
                    ...
                    variant="secondary"
                    onClick={() => {
                      setSelected(c as typeof selected)
                      setModal("fatura")
                    }}
                  >
                    Ver Fatura
                  </Btn>
                  <Btn small onClick={() => setModal("checkout")}>
                    Check-out
                  </Btn>
                </div>
              </div>
      ...
      <Card>
        ...
        <Table
          ...
          rows={hospedagens.map((h) => [
            ...
            <Badge
              label={h.status}
              color={h.status === "Ativo" ? "green" : "gray"}
            />,
          ])}
        />
      </Card>
        ...
        <Modal title="Realizar Check-in" onClose={() => setModal(null)}>
          ...
          <div className="space-y-3">
            <div className="p-3 rounded-lg" style={{ background: "#f9fafb" }}>
              ...
              >
                <strong>Hóspede:</strong> {selected.hospede}
              </p>
              <p
                className="text-sm"
                style={{ fontFamily: '"Inria Serif:Regular", serif' }}
              >
                <strong>Quarto:</strong> {selected.quarto}
              </p>
              <p
                className="text-sm"
                style={{ fontFamily: '"Inria Serif:Regular", serif' }}
              >
                <strong>Período:</strong> {selected.checkin} →{" "}
                {selected.checkout}
              </p>
            </div>
            <Input
