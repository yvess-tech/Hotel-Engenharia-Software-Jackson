...
const usuarios = [
  { id: 1, nome: "João Carlos", email: "joao@hotel.com", perfil: "Administrador", status: "Ativo", ultimo: "13/09/2026 09:00" },
  { id: 2, nome: "Carlos Mendes", email: "carlos@hotel.com", perfil: "Recepcionista", status: "Ativo", ultimo: "13/09/2026 08:05" },
  { id: 3, nome: "Fernanda Lima", email: "fernanda@hotel.com", perfil: "Financeiro", status: "Inativo", ultimo: "10/09/2026 17:00" },
];

const perfis = [
  { nome: "Administrador", permissoes: "Total", usuarios: 1, status: "Ativo" },
  { nome: "Recepcionista", permissoes: "Reservas, Check-in/out, Hóspedes", usuarios: 3, status: "Ativo" },
  { nome: "Camareira", permissoes: "Limpeza", usuarios: 2, status: "Ativo" },
  { nome: "Financeiro", permissoes: "Financeiro, Relatórios", usuarios: 1, status: "Ativo" },
];

const logs = [
  { usuario: "João Carlos", acao: "Login", modulo: "Sistema", data: "13/09/2026 09:00" },
  { usuario: "Carlos Mendes", acao: "Check-in", modulo: "Reservas", data: "13/09/2026 08:10" },
  { usuario: "João Carlos", acao: "Cadastro hóspede", modulo: "Hóspedes", data: "13/09/2026 09:15" },
  { usuario: "Fernanda Lima", acao: "Login falhou", modulo: "Sistema", data: "10/09/2026 17:05" },
];
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={tab === "usuarios" ? <Btn onClick={() => setModal(true)}>+ Cadastrar Usuário</Btn> : undefined}
      />
      ...
      {tab === "usuarios" && (
        <Card>
          <Table
            headers={["Nome", "E-mail", "Perfil", "Último acesso", "Status", "Ações"]}
            rows={usuarios.map((u) => [
              u.nome,
              u.email,
              u.perfil,
              u.ultimo,
              <Badge label={u.status} color={u.status === "Ativo" ? "green" : "gray"} />,
              <div className="flex gap-1">
                <Btn small variant="ghost">Editar</Btn>
                <Btn small variant="secondary">Senha</Btn>
                <Btn small variant="danger">Inativar</Btn>
              </div>,
            ])}
          />
        </Card>
